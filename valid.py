...
if args["arch"] == 'detr':
    ...
    #feature_extractor = DetrFeatureExtractor.from_pretrained('/home/jpkao/CVPDL/object_detection/ckpt/lightning_logs/version_5/checkpoints/epoch=99-step=3800.ckpt')
...
class MyLightningModule(L.LightningModule):
    ...
    def on_train_epoch_end(self):
        ...
        self.log("training_epoch_mean", epoch_mean)
        self.training_step_outputs.clear()
#=====set max epoch=809 make sure go to evaluation directly
trainer = Trainer(accelerator='gpu', devices=1, gradient_clip_val=0.1,max_epochs=809)
...
#======== modified from train.py for generating valid.json to make sure my output format and performance
print("Running evaluation...")
#=======================================
class val_loader(torchvision.datasets.CocoDetection):
    def __init__(self, img_folder, feature_extractor, train=True):
        ann_file = os.path.join(img_folder,  "val.json")
